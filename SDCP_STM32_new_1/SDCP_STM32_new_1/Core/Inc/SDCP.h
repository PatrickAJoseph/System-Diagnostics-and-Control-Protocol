...
/* Block transfer buffer max. size */

#define SDCP_BTR_BUFFER_SIZE		32

/* Block transfer parameter standard data types. */
...
volatile uint32_t SDCP_BTR_PCNTR;			//	Block transfer: parameter counter.
volatile uint32_t SDCP_BTR_XCNTR;        //  Block transfer: transfer counter.
volatile uint32_t SDCP_BTR_currentPID;	//	Block transfer: current PID.

uint8_t SDCP_BTR_PID[9];			//	Block transfer: Parameters IDs to transfer.
uint8_t SDCP_BTR_buffer[SDCP_BTR_BUFFER_SIZE];
uint8_t SDCP_BTR_buffer_position;
...
/* Function to push data into the block transfer buffer. */
